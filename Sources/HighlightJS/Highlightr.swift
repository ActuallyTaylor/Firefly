...
open class Highlightr
{
    ...
    private func processHTMLString(_ string: String) -> NSAttributedString? {
        ...
        while !scanner.isAtEnd {
            ...
            if let str = scanner.scanUpToString(htmlStart) {
            ...
            //str.index(startIndex, offsetBy: 1)
            let string = scanner.string
            let nextChar = string[scanner.currentIndex..<string.index(after: scanner.currentIndex)]
            if(nextChar == "s") {
                scanner.currentIndex = string.index(scanner.currentIndex, offsetBy: (spanStart as NSString).length)
                scannedString = scanner.scanUpToString(spanStartClose) as NSString?
                scanner.currentIndex = string.index(scanner.currentIndex, offsetBy: (spanStartClose as NSString).length)

                propStack.append(scannedString! as String)
            } else if(nextChar == "/") {
                scanner.currentIndex = string.index(scanner.currentIndex, offsetBy: (spanEnd as NSString).length)
