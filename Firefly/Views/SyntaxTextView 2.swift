...
open class SyntaxTextView: UIView {
    ...
    private func createInnerTextView() -> InnerTextView {
        ...
        textStorage.highlightr.setTheme(to: defaultThemeName, fontName: fontName)
    ...
    @discardableResult
    public func setTheme(name: String, fontName: String) -> Theme {
        let bundle = Bundle(for: SyntaxTextView.self)
        guard let defTheme = bundle.path(forResource: name+".min", ofType: "css") else { return  Theme(themeString: "", fontName: "")}
        let themeString = try! String.init(contentsOfFile: defTheme)
        let newTheme = Theme(themeString: themeString, fontName: fontName)
        self.theme = newTheme
